...
// Returns the completed-age in years (e.g. "32") or '' if it can't be parsed.
...
export const Individual_Details = ({ children, open, close, mobileNumber = '' }) => {
  ...
  // Single-select toggle for "Tobacco / Smoking Usage".
  const handleTobaccoPick = (value) => {
    setHealthInfo((prev) => ({
      ...prev,
      tobaccoUsage: prev.tobaccoUsage === value ? '' : value,
    }));
  };
  ...
  const goBack = () => {
    if (step === Step.InsuranceDetails) {
      ...
      setHealthInfo({
        ...
        tobaccoUsage: details.tobaccoUsage,
      });
      setForm((prev) => ({
        ...prev,
        fullName: details.fullName,
  ...
  const handleSubmitHealthInfo = () => {
    ...
    const tobaccoUsage = (HealthInfo.tobaccoUsage || '').trim();
    ...
    if (!tobaccoUsage) {
      window.alert('Please confirm tobacco / smoking usage');
      return;
    }
  ...
  const summaryFields = [
    ...
    { key: 'tobaccoUsage', label: 'Tobacco Usage', value: details.tobaccoUsage, icon: HiOutlineClipboardCheck, editable: true, step: Step.HealthInfo },
  ];
  ...
  const renderHealthInfo = () => (
    <section className="sr-form sr-step sr-step--health" data-step="health">
      ...
      <div className="sr-form__body">
        ...
        {/* DIV 3 — Tobacco / Smoking Usage (Yes/No single-select) */}
        <div className="sr-block">
          <h3 className="sr-block__heading">
            <HiOutlineClipboardCheck aria-hidden="true" />
            {HEALTH_INFO.label3}
          ...
          <div className="sr-chips sr-chips--col3">
            {HEALTH_INFO.tobaccoOptions.map((value) => {
              const selected = HealthInfo.tobaccoUsage === value;
              return (
                <button
                  type="button"
                  key={value}
                  className={`sr-chip${selected ? ' is-selected' : ''}`}
                  onClick={() => handleTobaccoPick(value)}
                  aria-pressed={selected}
                >
                  {value}
                </button>
      ...
      <div className="sr-form__body">
        ...
        <div className="sr-review">
          ...
          <ul className="sr-review__list">
            ...
            <li className="sr-review__item">
              <span className="sr-review__label">Tobacco / Smoking Usage</span>
              <span className="sr-review__value">{details.tobaccoUsage || '--'}</span>
            </li>
          </ul>
        </div>
      </div>
